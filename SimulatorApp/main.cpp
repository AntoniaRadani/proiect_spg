...
int main() {
    ...
    unsigned int fabricTex = loadTexture("Textures/cloth3.jpg");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (isRunning) {
            // Evităm împărțirea la 0 în caz că pânza este scalată la minim
            float safeClothScale = std::max(0.001f, clothScale);
            float relativeScale = (1.0f / safeClothScale);

            // Ajustăm raza și poziția de coliziune trimise către simulator
            float collisionRadius = (sphereRadius * sphereScale) * relativeScale;
            glm::vec3 collisionPos = spherePos * relativeScale;

            // Clădim pașii de simulare fizică (folosim un pas de timp fix de maxim 0.02s pentru stabilitate)
            float fixedDeltaTime = std::min(deltaTime, 0.02f);
            cloth.simulate(fixedDeltaTime, collisionPos, collisionRadius, 5);
        }
